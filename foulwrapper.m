...
NSString* foulwrapper(int argc, char *argv[])
{
    ...
    [fm removeItemAtPath:@"/var/tmp/decrypted" error:nil];
    ...
    if (argc != 5 && !serverMode) {
        ...
        [fm createDirectoryAtPath:@"/var/tmp/decrypted" withIntermediateDirectories:true attributes:[NSDictionary dictionaryWithObject:@0777 forKey:NSFilePosixPermissions] error:nil];
        webserver(); // the JANK
        exit(0);
    } else {
    ...
    [fm createDirectoryAtPath:[archivePath path] withIntermediateDirectories:true attributes:[NSDictionary dictionaryWithObject:@0777 forKey:NSFilePosixPermissions] error:nil];
    ...
    [fm removeItemAtPath:@"/var/tmp/fouldecrypt" error:nil];
    if (hasIPA) {
        LSApplicationWorkspace *workspace = [LSApplicationWorkspace defaultWorkspace];
